...
describe("readCompanyHistory", () => {
	it(
		"names every contact at the company, with their id",
		scoped(async () => {
			...
			expect(
				history?.people.find((person) => person.id === paulaId)?.title,
			).toBe("Growth Specialist");
		}),
	);

	it(
		"flags a contact still named after their email address",
		scoped(async () => {
			const history = await readCompanyHistory(companyId);
	...
	it(
		"returns the deals with stage, value and who is on them",
		scoped(async () => {
			const history = await readCompanyHistory(companyId);
	...
	it(
		"reads the correspondence and knows they replied",
		scoped(async () => {
			const history = await readCompanyHistory(companyId);
	...
	it(
		"leaves email and meeting projections out of the notes",
		scoped(async () => {
			const history = await readCompanyHistory(companyId);
	...
	it(
		"returns null for a company that does not exist",
		scoped(async () => {
...
describe("readDealHistory", () => {
	it(
		"reports the stage clock, not just the stage",
		scoped(async () => {
	...
	it(
		"returns every stage it moved through, oldest first",
		scoped(async () => {
			const history = await readDealHistory(dealId);
	...
	it(
		"names who is on it, with ids and roles",
		scoped(async () => {
			const history = await readDealHistory(dealId);
	...
	it(
		"says the correspondence is the account's, not the deal's",
		scoped(async () => {
			const history = await readDealHistory(dealId);
	...
	it(
		"returns null for a deal that does not exist",
		scoped(async () => {
