...
describe("recordFact", () => {
	it(
		"writes a verified fact through to the record",
		scoped(async () => {
	...
	it(
		"keeps a probable fact off the record and offers it instead",
		scoped(async () => {
			const result = await recordFact({
				contactId,
				field: "twitterUrl",
	...
	it(
		"never re-offers a value a person dismissed",
		scoped(async () => {
	...
	it(
		"supersedes rather than replaces, which is how a job change is noticed",
		scoped(async () => {
			await recordFact({
				contactId,
				field: "employer",
			...
			expect(facts.find((f) => f.value === "Fleetio")?.status).toBe(
				"SUPERSEDED",
			);
	...
	it(
		"stores the evidence, so the score can be explained later",
		scoped(async () => {
...
describe("writeBrief", () => {
	...
	it(
		"refuses a brief nothing supports",
		scoped(async () => {
