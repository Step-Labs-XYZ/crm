...
describe("claimDue", () => {
	...
	it(
		"does not hand the same row to two dispatchers",
		scoped(async () => {
	...
	it(
		"leaves work that is not due yet",
		scoped(async () => {
	...
	it(
		"does not re-claim a leased row, and does re-claim an expired one",
		scoped(async () => {
			const task = await queue();
	...
	it(
		"stops handing out a row that has spent its attempts",
		scoped(async () => {
			const task = await queue();
	...
	it(
		"counts the attempts it has handed out",
		scoped(async () => {
			const task = await queue();
	...
	it(
		"stops claiming once the work is finished",
		scoped(async () => {
			const task = await queue();
			await claimDue(10, RESEARCH);
...
describe("retireExhausted", () => {
	it(
		"gives up on a row that never reported back, and says who it was about",
		scoped(async () => {
	...
	it(
		"leaves a row that is still leased on its last attempt alone",
		scoped(async () => {
			const task = await queue();
	...
	it(
		"leaves work that still has attempts left",
		scoped(async () => {
			await queue();
			await claimDue(10, RESEARCH);
...
describe("completeTask", () => {
	it(
		"retires a row once, and reports who it was about",
		scoped(async () => {
			const contact = await someone();
			const task = await queue({ contactId: contact.id });
			await claimDue(10, RESEARCH);
...
describe("scheduleTask", () => {
	it(
		"books work with the agent's own reason",
		scoped(async () => {
	...
	it(
		"moves the existing booking rather than queueing a second one",
		scoped(async () => {
